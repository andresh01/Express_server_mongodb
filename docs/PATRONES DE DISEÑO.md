...
console.log("New instance", instanceof(obj));  //Authentication class
...
module.export = Authentication; // 0x086F crea una un espacaio de memoria

// file1.js

const auth = new require("./autentication")(); // 00x086F se crea una instancia desde un archivo nuevo y apunto hacia esa misma mmemoria

// file2.js

const auth = new require("./autentication")(); // 00x086F se crea una instancia desde otro archivo diferente pero igual esta apuntando hacia el mismo espacio de memoria- esa es la caracteristica de singleton
```
...
//adapata las propiedades del objeto a los requerimientos ejemplo el pasword deberia ser privado y no mostrasrelo o encriptarlo

class UserAdapter{
    static factoryAll(users){
        return usuers.map({username, id, email} => {
